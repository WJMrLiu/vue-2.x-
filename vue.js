...
(function(global, factory) {
  ...
  function parse(template, options) { // 该函数的主要作用是把 template 字符串模板 ， 转成 ast
    ...
    /**
     * parseHTML 处理 template 匹配标签， 再传入 start， end， charts 等方法
     */
    parseHTML(template, {
      warn: warn$2,
      ...
      start: function start(tag, attrs, unary, start$1, end) { // 每当 parseHTML 匹配到一个 首标签，都会把该标签的信息传给 start 方法，让他来处理
        ...
        if (options.outputSourceRange) {
          element.end = end$1; // 重新设置 stack 最后一个节点为父节点
        }
        closeElement(element);
      },
      /**
  ...
  function markStatic$1(node) {
    node.static = isStatic(node); // 进行初步静态节点判断
    if (node.type === 1) {
  ...
  function isStatic(node) {
    ...
    return !!(
      ...
      isPlatformReservedTag(node.tag) && // not a component 正常 html 标签
  ...
  var createCompiler = createCompilerCreator(function baseCompile(
    ...
    if (options.optimize !== false) {
      optimize(ast, options); // optimize 标记静态节点
    }
